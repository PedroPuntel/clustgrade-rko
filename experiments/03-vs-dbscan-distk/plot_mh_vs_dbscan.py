...
def main() -> None:
    ...
    msi_mh_wins = (
        msi_valid.sort_values("msi_delta", ascending=False).head(PANEL_COUNT).reset_index(drop=True)
    )
    ...
    _scatter_ranked_instances(
        ...
        f"plot-01-{best_mh}-top5-msi-win.png",
        context,
    )
    _scatter_ranked_instances(
        msi_mh_wins,
        ...
        "plot-02-dbscan-top5-msi-win.png",
        context,
    )

    # --- Top-5 datasets where DBSCAN-DistK outperforms best MH (MSI) ---
    msi_db_wins = (
        msi_valid.sort_values("msi_delta", ascending=True).head(PANEL_COUNT).reset_index(drop=True)
    )
    ...
    _scatter_ranked_instances(
        ...
        "plot-05-dbscan-top5-msi-loss.png",
        context,
    )
    _scatter_ranked_instances(
        msi_db_wins,
        ...
        f"plot-06-{best_mh}-top5-msi-loss.png",
        context,
    )
    ...
    if len(ari_valid) > 0:
        ari_mh_wins = (
            ari_valid.sort_values("ari_delta", ascending=False)
            .head(PANEL_COUNT)
            .reset_index(drop=True)
        )
        ...
        _scatter_ranked_instances(
            ...
            f"plot-03-{best_mh}-top5-ari-win.png",
            context,
        )
        _scatter_ranked_instances(
            ari_mh_wins,
            "DBSCAN",
            "Exp 03: DBSCAN-DistK on same datasets (comparison)",
            "plot-04-dbscan-top5-ari-win.png",
            context,
        )
